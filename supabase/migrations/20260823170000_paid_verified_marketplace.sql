...
drop policy if exists "portal_verification_storage_insert_own" on storage.objects;
create policy "portal_verification_storage_insert_own"
  on storage.objects for insert to authenticated
  with check (
    bucket_id = 'portal-verification'
    and (storage.foldername(name))[1] = (select auth.uid())::text
  );

drop policy if exists "portal_verification_storage_select_own" on storage.objects;
create policy "portal_verification_storage_select_own"
...
drop policy if exists "portal_verification_storage_delete_own" on storage.objects;
create policy "portal_verification_storage_delete_own"
