  ...
  return (
    <section className="min-h-[80vh] bg-slate-50 py-10">
      <div className="container-prose max-w-3xl">
        <a href="/portal/panel" className="text-sm font-semibold text-teal hover:text-gold">← Portala dön</a>
          ...
          <CardContent className="p-6 md:p-8">
            {submitted ? (
              <div className="py-8 text-center">
                ...
                <h2 className="mt-4 font-display text-2xl font-semibold text-navy">Başvuru alındı</h2>
                <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
                  Belgeniz herkese açık değildir. Sonuç panelinizde gösterilecek; gerekirse ek belge istenir.
                </p>
                <Button asChild className="mt-6 bg-navy text-white"><a href="/portal/panel">Panele dön</a></Button>
              </div>
            ) : (
              <>
                <div className="grid gap-5 sm:grid-cols-2">
                  <label className="text-sm font-semibold text-navy">
                    Hesap türü
                    <select value={role} onChange={(event) => setRole(event.target.value as typeof role)} className="mt-2 h-12 w-full rounded-xl border bg-white px-3">
                  ...
                  <label className="text-sm font-semibold text-navy">
                    Belge türü
                    <select value={documentType} onChange={(event) => setDocumentType(event.target.value)} className="mt-2 h-12 w-full rounded-xl border bg-white px-3">
                ...
                <label className="mt-6 block rounded-2xl border-2 border-dashed border-teal/30 bg-teal/5 p-6 text-center">
                  ...
                  <span className="mt-3 block font-semibold text-navy">Belgeyi güvenli alana yükle</span>
                  <span className="mt-1 block text-xs text-muted-foreground">PDF, JPG veya PNG · en fazla 8 MB</span>
                  <input
                    type="file"
                ...
                <div className="mt-5 rounded-xl border bg-slate-50 p-4 text-xs leading-relaxed text-muted-foreground">
                  ...
                  Belge yalnız yetkili inceleme ekibi tarafından görülür; public bağlantı oluşturulmaz.
                  KVKK/GDPR kapsamında veri minimizasyonu ve saklama süresi uygulanır.
                </div>

                <Button onClick={submit} disabled={submitting || !file} className="mt-6 w-full bg-gold text-white hover:bg-gold/90" size="lg">
