...
def test_provider_defaults_are_local(monkeypatch):
    for key in ("CLINIGA_LITELLM_URL", "CLINIGA_OLLAMA_URL", "CLINIGA_STAGEHAND_ENDPOINT", "CLINIGA_N8N_WEBHOOK"):
...
def test_litellm_precedes_ollama(monkeypatch):
    monkeypatch.setenv("CLINIGA_LITELLM_URL", "http://litellm.local/v1")
    monkeypatch.setenv("CLINIGA_OLLAMA_URL", "http://ollama.local")
    router = RuntimeProviderRouter()
    assert router.decide("general reasoning").provider == "litellm"


def test_external_actions_require_explicit_prefix(monkeypatch):
    monkeypatch.setenv("CLINIGA_STAGEHAND_ENDPOINT", "http://stagehand.local/run")
    monkeypatch.setenv("CLINIGA_N8N_WEBHOOK", "http://n8n.local/webhook")
    router = RuntimeProviderRouter()
